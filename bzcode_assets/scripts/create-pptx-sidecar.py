...
def _export_pptx(out_path: Path, slides: list[dict]) -> None:
    ...
    def align(s):
        if s == "center": return PP_ALIGN.CENTER
        if s == "right":  return PP_ALIGN.RIGHT
    ...
    PRST = {
        ...
        "plus": "plus", "rightArrow": "rightArrow", "leftArrow": "leftArrow",
        "upArrow": "upArrow", "downArrow": "downArrow",
        "chevron": "chevron", "star4": "star4", "star5": "star5",
        "snip1Rect": "snip1Rect",
    }
    ...
    def apply_prst(sp, st, cr=0):
        ...
        if st == "roundRect" and cr > 0:
            ...
            g2.set("name", "adj"); g2.set("fmla", f"val {adj}")
    ...
    def apply_fill(sp, fill, bs):
        ...
        if bg_grad: grad = bg_grad; ft = "gradient"
            ...
            try:
                ...
                if sp_pr is not None:
                    for tag in ("a:solidFill", "a:gradFill", "a:noFill", "a:blipFill"):
                        for el in sp_pr.findall(qn(tag)): sp_pr.remove(el)
    ...
    def apply_line(sp, bs):
        ...
        if bc and bc != "transparent" and bw > 0:
            try:
                rgb = hex_rgb(bc)
                if rgb: sp.line.color.rgb = rgb; sp.line.width = Pt(bw)
            except Exception: pass
        else:
            try: sp.line.fill.background()
            except Exception: pass
    ...
    def apply_text(tf, paragraphs, plain, bs, font):
        if paragraphs:
            for pi, pd in enumerate(paragraphs):
                ...
                try: para.alignment = align(pd.get("align") or (bs or {}).get("textAlign", "left"))
                except Exception: pass
                try:
                ...
                for rd in (pd.get("runs") or []):
                    ...
                    if rd.get("bold"): run.font.bold = True
                    if rd.get("italic"): run.font.italic = True
                    if rd.get("underline"): run.font.underline = True
            ...
            for li, line in enumerate((plain or "").split("\n")):
                ...
                run.font.name = font
                if (bs or {}).get("fontWeight") == "bold": run.font.bold = True
    ...
    def apply_anchor(tf, bs):
        ...
        if anchor:
            try:
                ...
                if bp is not None: bp.set("anchor", anchor)
            except Exception: pass
    ...
    for sd in slides:
        ...
        try:
            ...
            if bg_grad and bg_grad.get("stops"):
                ...
                if bg_pr is None: bg_pr = etree.SubElement(bg_el, qn("p:bg"))
                bg_prop = bg_pr.find(qn("p:bgPr"))
                if bg_prop is None: bg_prop = etree.SubElement(bg_pr, qn("p:bgPr"))
                for tag in ("a:solidFill", "a:gradFill", "a:noFill"):
                    for el in bg_prop.findall(qn(tag)): bg_prop.remove(el)
        ...
        for box in sd.get("boxes", []):
            try:
                ...
                if fill and not has_text and st not in ("", "textbox"):
                    ...
                    apply_line(sp, bs)
                    if rot: sp.rotation = rot
                    if sp.has_text_frame: sp.text_frame.text = ""
                    continue
                ...
                if bgc and bgc != "transparent":
                    rgb = hex_rgb(bgc)
                    if rgb: tb.fill.solid(); tb.fill.fore_color.rgb = rgb
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build a PPTX from template sidecar JSON + content spec."
    )
    parser.add_argument("--template-sidecar", required=True,
                        help="Path to template sidecar JSON (.<name>.pptx.json).")
