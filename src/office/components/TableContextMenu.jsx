...
const TableContextMenu = forwardRef(({
  x, y, visible,
  onClose,
  onDeleteRow, onDeleteColumn, onAddRow, onAddColumn, onDeleteTable,
}, ref) => {
  const menuRef = useRef(null);
  useImperativeHandle(ref, () => menuRef.current);

  useClickOutside(menuRef, () => { if (visible) onClose(); });
  ...
  const item = (label, handler) => (
    <button
      ...
      style={{
        ...
        color: 'var(--text-primary, #111)', fontFamily: 'inherit',
      }}
  ...
  const divider = <div style={{ height: 1, background: 'var(--border-default, rgba(255,255,255,0.12))', margin: '3px 0' }} />;
