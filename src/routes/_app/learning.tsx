...
const BAL_WEIGHTS = { task: 0.30, preference: 0.25, domain: 0.20, efficiency: 0.15, consistency: 0.10 };

const balTotal = (c: { task: number; preference: number; domain: number; efficiency: number; consistency: number }) =>
  +(BAL_WEIGHTS.task * c.task + BAL_WEIGHTS.preference * c.preference + BAL_WEIGHTS.domain * c.domain +
    BAL_WEIGHTS.efficiency * c.efficiency + BAL_WEIGHTS.consistency * c.consistency).toFixed(3);
...
const MOCK_SESSIONS = [
  {
    ...
    status: 'completed',
    rounds: 10, tokens: 4812, epochs: 3, duration: '1m 48s',
    // per-epoch BAL component breakdown
    epochLoss: [
      { task: 0.712, preference: 0.831, domain: 0.654, efficiency: 0.720, consistency: 0.890 },
      { task: 0.501, preference: 0.544, domain: 0.430, efficiency: 0.480, consistency: 0.610 },
      { task: 0.218, preference: 0.285, domain: 0.312, efficiency: 0.340, consistency: 0.390 },
    ],
    metrics: { accuracy: 84.6, quality: 91.3, efficiency: '+18.2%', adaptation: '+11.5%', latency: '1.4s' },
    baselineAccuracy: 71.2,
    notes: 'Best session so far. Task Loss dropped sharply — 3 high-quality code refactoring rounds provided clear completion signals. Preference Loss converged fastest, suggesting the agent has internalised this user\'s preferred response depth.',
  },
  {
    id: 4,
    date: '2026-06-14 09:15',
    status: 'completed',
    rounds: 8, tokens: 3644, epochs: 3, duration: '1m 22s',
    epochLoss: [
      { task: 0.801, preference: 0.755, domain: 0.820, efficiency: 0.680, consistency: 0.740 },
      { task: 0.620, preference: 0.590, domain: 0.610, efficiency: 0.510, consistency: 0.560 },
      { task: 0.412, preference: 0.430, domain: 0.521, efficiency: 0.390, consistency: 0.448 },
    ],
    metrics: { accuracy: 78.3, quality: 83.1, efficiency: '+12.4%', adaptation: '+8.7%', latency: '1.6s' },
    baselineAccuracy: 71.2,
    notes: 'Steady improvement across all components. Domain Loss remains higher than Task Loss — the agent is still building knowledge of this user\'s specific document formats. Consistency Loss improved significantly, reflecting more predictable behaviour on follow-up questions.',
  },
  {
    id: 3,
    date: '2026-06-13 16:40',
    status: 'completed',
    rounds: 12, tokens: 5901, epochs: 2, duration: '2m 05s',
    epochLoss: [
      { task: 0.870, preference: 0.910, domain: 0.840, efficiency: 0.760, consistency: 0.920 },
      { task: 0.581, preference: 0.630, domain: 0.592, efficiency: 0.541, consistency: 0.680 },
    ],
    metrics: { accuracy: 74.8, quality: 79.4, efficiency: '+7.1%', adaptation: '+5.2%', latency: '1.9s' },
    baselineAccuracy: 71.2,
    notes: 'Early stopping triggered after epoch 2 — BAL was still decreasing but below the minimum improvement threshold. High Consistency Loss suggests mixed-topic conversations confused the agent about user intent. Efficiency Loss improved least, indicating the agent is still over-explaining.',
  },
  {
    id: 2,
    date: '2026-06-12 11:30',
    status: 'failed',
    rounds: 6, tokens: 2103, epochs: 1, duration: '0m 41s',
    epochLoss: [
      { task: 0.944, preference: 0.881, domain: 0.912, efficiency: 0.870, consistency: 0.950 },
    ],
    metrics: { accuracy: 63.1, quality: 61.2, efficiency: '-2.1%', adaptation: '+0.3%', latency: '2.3s' },
    baselineAccuracy: 71.2,
    notes: 'Job failed mid-training — only 6 rounds, too few for BAL to converge. Preference Loss and Consistency Loss barely moved from baseline. Minimum 8 rounds recommended for meaningful alignment signal.',
  },
  {
    id: 1,
    date: '2026-06-11 08:00',
    status: 'completed',
    rounds: 5, tokens: 1820, epochs: 3, duration: '0m 58s',
    epochLoss: [
      { task: 0.980, preference: 0.970, domain: 0.990, efficiency: 0.920, consistency: 0.960 },
      { task: 0.841, preference: 0.812, domain: 0.880, efficiency: 0.790, consistency: 0.840 },
      { task: 0.681, preference: 0.660, domain: 0.730, efficiency: 0.641, consistency: 0.710 },
    ],
    metrics: { accuracy: 63.4, quality: 68.0, efficiency: '+3.8%', adaptation: '+2.1%', latency: '2.1s' },
    baselineAccuracy: 58.1,
    notes: 'First session — established baseline BAL of ~0.95 across all components. Small dataset (5 rounds) but loss converged cleanly across all 3 epochs. Task Loss led the way; Domain and Consistency Loss will need more data to move significantly.',
  },
];
  ...
  return (
    <div className="ll-page">
      ...
      <div className="ll-header">
        <div className="ll-header-left">
          ...
          <div>
            ...
            <p className="ll-subtitle">Training data collected from your conversations · last job 2h ago</p>
          </div>
        </div>
      ...
      <div className="ll-tabs">
        <button className={`ll-tab${tab === 'data' ? ' ll-tab--active' : ''}`} onClick={() => setTab('data')}>
          Training Data
        </button>
        <button className={`ll-tab${tab === 'eval' ? ' ll-tab--active' : ''}`} onClick={() => setTab('eval')}>
          Latest Evaluation
        </button>
        <button className={`ll-tab${tab === 'sessions' ? ' ll-tab--active' : ''}`} onClick={() => setTab('sessions')}>
          Training Sessions
        </button>
      </div>
        ...
        <div className="ll-rounds">
          {MOCK_ROUNDS.map(r => (
            <div key={r.id} className={`ll-round${expanded === r.id ? ' ll-round--open' : ''}`}>
              <div className="ll-round-header" onClick={() => setExpanded(expanded === r.id ? null : r.id)}>
                <span className="ll-round-num">#{r.id}</span>
                <span className="ll-round-ts">{r.ts}</span>
                <span className="ll-round-preview">{r.user.slice(0, 70)}{r.user.length > 70 ? '…' : ''}</span>
                <div className="ll-round-tags">
                  {r.tags.map(t => <span key={t} className="ll-tag">{t}</span>)}
                </div>
                <div className="ll-quality" style={{ color: QUALITY_COLOR(r.quality) }}>
                  {r.quality}
                </div>
                <svg className={`ll-chevron${expanded === r.id ? ' ll-chevron--open' : ''}`} viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" strokeWidth="2" fill="none"><polyline points="6 9 12 15 18 9" /></svg>
              </div>
      ...
      {tab === 'eval' && (
        <div className="ll-eval">
          <div className="ll-eval-banner">
            <div className="ll-eval-banner-icon"><BrainIcon size={18} /></div>
            <div>
          ...
          <div className="ll-eval-grid">
            {[
              { label: 'Baseline Accuracy',  val: `${MOCK_EVAL.baselineAccuracy}%`, sub: 'before training' },
              { label: 'New Accuracy',        val: `${MOCK_EVAL.newAccuracy}%`,     sub: 'after training', highlight: true },
              { label: 'Loss',                val: MOCK_EVAL.loss,                  sub: 'cross-entropy' },
              { label: 'Perplexity',          val: MOCK_EVAL.perplexity,            sub: 'lower is better' },
              { label: 'Response Quality',    val: `${MOCK_EVAL.responseQuality}%`, sub: 'human eval score', highlight: true },
              { label: 'Domain Adaptation',  val: MOCK_EVAL.domainAdaptation,      sub: 'vs base model', highlight: true },
          ...
          <div className="ll-sparkline">
            {[58.1, 63.4, 71.2, 84.6].map((v, i) => (
              <div key={i} className="ll-sparkline-bar-wrap">
                <div className="ll-sparkline-bar" style={{ height: `${v}%`, background: i === 3 ? 'var(--accent-blue)' : 'var(--bg-tertiary)' }} />
      ...
      {tab === 'sessions' && (
        <div className="ll-sessions">
          <div className="ll-bal-legend">
            ...
            <span className="ll-bal-formula">= 0.30·L<sub>task</sub> + 0.25·L<sub>pref</sub> + 0.20·L<sub>domain</sub> + 0.15·L<sub>eff</sub> + 0.10·L<sub>cons</sub></span>
            <span className="ll-bal-note">Lower = more aligned with your usage. Measured on conversation outcomes, not token prediction.</span>
          </div>
          ...
          {MOCK_SESSIONS.map(s => {
            ...
            const statusColor = s.status === 'completed' ? 'var(--accent-green)' : 'var(--accent-red)';
            return (
              <div key={s.id} className="ll-session" onClick={() => setSelectedSession(s)} style={{ cursor: 'pointer' }}>
                <div className="ll-session-row">
                  ...
                  <div className="ll-session-bal">
                    ...
                    <span className="ll-session-bal-to" style={{ color: finalBAL < firstBAL ? 'var(--accent-green)' : 'var(--accent-red)' }}>{finalBAL}</span>
                  </div>
                  <div className="ll-session-gain" style={{ color: gain >= 0 ? 'var(--accent-green)' : 'var(--accent-red)' }}>
                    {gain >= 0 ? '+' : ''}{gain.toFixed(1)}%
                  </div>
                  <svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" strokeWidth="2" fill="none" style={{ opacity: 0.4, flexShrink: 0 }}><polyline points="9 18 15 12 9 6" /></svg>
                </div>
              </div>
            );
          })}
...
const BAL_COMPONENTS_DEF = [
  { key: 'task'        as const, label: 'Task Completion',   weight: '30%', desc: 'Did the agent fully complete what was asked?' },
  { key: 'preference'  as const, label: 'Preference Align',  weight: '25%', desc: "Matches the user's observed style and depth preferences" },
  { key: 'domain'      as const, label: 'Domain Knowledge',  weight: '20%', desc: 'Accuracy within the user\'s specific knowledge domain' },
  { key: 'efficiency'  as const, label: 'Response Efficiency', weight: '15%', desc: 'Avoids unnecessary verbosity or unhelpful brevity' },
  { key: 'consistency' as const, label: 'Consistency',       weight: '10%', desc: 'Coherent behaviour across similar follow-up requests' },
];
    ...
    <div className="ll-page">
      ...
      <div className="ll-detail-header">
        <button className="ll-detail-back" onClick={onBack}>
          <svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" strokeWidth="2.5" fill="none"><polyline points="15 18 9 12 15 6" /></svg>
          Training Sessions
        </button>
        <div className="ll-detail-title-block">
          ...
          <div className="ll-detail-meta">{session.date} · {session.rounds} rounds · {session.tokens.toLocaleString()} tokens · {session.epochs} epochs · {session.duration}</div>
        </div>
        <div className="ll-detail-bal-badge">
          ...
          <span style={{ color: finalBAL < firstBAL ? 'var(--accent-green)' : 'var(--accent-red)', fontWeight: 700 }}>{finalBAL}</span>
        </div>
        <div style={{ color: gain >= 0 ? 'var(--accent-green)' : 'var(--accent-red)', fontWeight: 700, fontFamily: 'var(--font-heading)', fontSize: 18 }}>
          {gain >= 0 ? '+' : ''}{gain.toFixed(1)}% accuracy
        </div>
      </div>
      ...
      <div className="ll-detail-body">
        ...
        <section className="ll-detail-section">
          ...
          <div className="ll-eval-grid">
            {[
              { label: 'Accuracy',          val: `${session.metrics.accuracy}%`,  sub: 'after training',   good: true  },
              { label: 'Response Quality',  val: `${session.metrics.quality}%`,   sub: 'human eval score', good: true  },
              { label: 'Efficiency',        val: session.metrics.efficiency,       sub: 'vs baseline',      good: true  },
              { label: 'Domain Adaptation', val: session.metrics.adaptation,       sub: 'vs base model',    good: true  },
              { label: 'Avg Latency',       val: session.metrics.latency,          sub: 'median response',  good: false },
            ].map(m => (
        ...
        <section className="ll-detail-section">
          ...
          <div className="ll-bal-legend" style={{ marginBottom: 20 }}>
            <span className="ll-bal-formula" style={{ fontSize: 13 }}>BAL = 0.30·L<sub>task</sub> + 0.25·L<sub>pref</sub> + 0.20·L<sub>domain</sub> + 0.15·L<sub>eff</sub> + 0.10·L<sub>cons</sub></span>
            <span className="ll-bal-note">Measured on actual conversation outcomes — not next-token prediction. Lower = more aligned with this user's needs.</span>
          </div>
              ...
              return (
                <div key={key} className="ll-bal-comp-card">
                  <div className="ll-bal-comp-card-top">
                    <span className="ll-bal-comp-card-name">L<sub>{key.slice(0, 4)}</sub></span>
                  ...
                  <div className="ll-bal-comp-card-vals">
                    {session.epochLoss.map((ep, i) => (
                      <div key={i} className="ll-bal-comp-card-epoch">
                        ...
                        <span className="ll-bal-comp-epoch-val" style={{ color: i > 0 && ep[key] < session.epochLoss[i - 1]![key] ? 'var(--accent-green)' : 'var(--text-primary)' }}>
                          {ep[key]}
                        </span>
                      </div>
                    ))}
                    <div className="ll-bal-comp-card-delta" style={{ color: delta < 0 ? 'var(--accent-green)' : 'var(--accent-red)' }}>
                      {delta < 0 ? '' : '+'}{delta}
                    </div>
                  </div>
                  {/* Mini bar showing convergence */}
                  <div className="ll-bal-comp-bar-track">
                    <div className="ll-bal-comp-bar-fill" style={{ width: `${finalEpoch[key] * 100}%` }} />
                  </div>
                </div>
              );
            })}
        ...
        <section className="ll-detail-section">
          ...
          <div className="ll-bal-table" style={{ marginBottom: 20 }}>
            <div className="ll-bal-table-header">
              ...
              {session.epochLoss.map((_, i) => <span key={i}>Epoch {i + 1}</span>)}
              <span>Δ total</span>
            </div>
            {BAL_COMPONENTS_DEF.map(({ key, weight }) => {
              ...
              return (
                <div key={key} className="ll-bal-table-row">
                  <span className="ll-bal-comp-name">L<sub>{key.slice(0, 4)}</sub> <span className="ll-bal-weight">{weight}</span></span>
                  {session.epochLoss.map((ep, i) => (
                    <span key={i} className="ll-bal-cell" style={{ color: i > 0 && ep[key] < session.epochLoss[i - 1]![key] ? 'var(--accent-green)' : 'var(--text-secondary)' }}>
                      {ep[key]}
                    </span>
                  ))}
                  <span className="ll-bal-delta" style={{ color: delta < 0 ? 'var(--accent-green)' : 'var(--accent-red)' }}>
                    {delta < 0 ? '' : '+'}{delta}
                  </span>
                </div>
              );
            ...
            <div className="ll-bal-table-row ll-bal-table-total">
              <span>BAL total</span>
              {session.epochLoss.map((ep, i) => <span key={i} className="ll-bal-cell">{balTotal(ep)}</span>)}
        ...
        <section className="ll-detail-section">
          ...
          <p style={{ fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.65, margin: 0 }}>{session.notes}</p>
        </section>
      </div>
    </div>
  );
