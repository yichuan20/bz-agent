...
ALLOWED_TYPES = {
    ...
    "boolean", "bool", "float", "double precision", "numeric", "real",
    "jsonb", "json", "uuid", "date", "timestamptz", "timestamp", "bytea",
}
...
async def cmd_list_tables(_args):
    ...
    try:
        ...
        """)
        out({"tables": [{"name": r["table_name"],
                         "columns": json.loads(r["columns"])} for r in rows]})
    finally:
        await conn.close()
...
async def cmd_query(args):
    ...
    try:
        ...
        out({"rows": [dict(r) for r in rows], "total": total,
...
async def cmd_delete(args):
    ...
    try:
        row = await conn.fetchrow(
            f"DELETE FROM {tbl} WHERE id = $1 RETURNING id", row_id
        )
        if row is None:
            die(f"Row id={row_id} not found in {args.table!r}")
        out({"deleted": row_id})
...
def build_parser():
    ...
    s = sub.add_parser("create-table",
                       help="Create a table  (id + created_at added automatically)")
    s.add_argument("table")
