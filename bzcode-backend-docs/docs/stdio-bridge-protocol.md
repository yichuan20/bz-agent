...
type ServerMessage = MessageMeta &
  ...
  // System events
  | { type: 'system'; event: string; message: string }
  // Result
...
**Important**: Queries are sequential. Do not send a new `user` message until the previous one completes (signaled by `result` followed by `status: idle`). If a query is already running, the server responds immediately with `{ "type": "result", "status": "error", "error": "A query is already running" }`. Permission and input responses (`subtype: "permission"` / `subtype: "input"`) are exempt — they must be sent while a query is running.
...
Sent when something happens that the UI should display but is not part of the conversation (e.g. context compaction, background operations).

```json
{ "type": "system", "event": "notification", "message": "Context compacted: 14 messages summarized, 6 preserved." }
```

The `message` field is human-readable and can be displayed directly (e.g. as a dimmed informational line). No client response is needed.
...
rl.on('line', (line) => {
  ...
  switch (msg.type) {
    ...
    case 'prompt':
      ...
      break
    case 'system':
      // Informational event (e.g. compaction notification)
...
| `--yolo` | Start in yolo mode (auto-approve all tool calls) |
| `--update [version]` | Update bzcode to the latest (or specified) version and exit |
...
| `-i, --identity <text>` | Override the agent identity prompt (takes priority over IDENTITY.md files) |
