...
function CredentialManager({ agentHttp, onClose }: { agentHttp: string; onClose: () => void }) {
  ...
  async function handleAdd() {
    ...
    setSaving(true);
    try {
      await setCredential(agentHttp, newKey.trim(), newVal.trim());
      setNewKey('');
      setNewVal('');
      refresh();
    } catch {
  ...
  async function handleDelete(key: string) {
    await deleteCredential(agentHttp, key).catch(() => {});
    refresh();
  }
...
function DirPickerPanel({
  ...
  function commitNewFolder() {
    ...
    makeDir(HTTP_BASE, browsePath, name)
...
function SessionListPage({
  ...
  function handleDelete(sessionId: string) {
    deleteSession(HTTP_BASE, sessionId)
...
function ConversationsPanel({
  ...
  function handleSetDefault(sessionId: string) {
    ...
    setDefaultSession(httpBase, cwd, next ?? '').catch(() => setDefaultSessionId(defaultSessionId)); // revert on error
  }
  ...
  async function handleDelete(sessionId: string, e: React.MouseEvent) {
    e.stopPropagation();
    await deleteSession(httpBase, sessionId).catch(() => null);
...
function AgentPage() {
  ...
  const startNewSession = useCallback(
    async (cwd: string, mode: AgentMode) => {
      ...
      try {
        const data = await connectSession(HTTP_BASE, { cwd, mode });
  ...
  const connectAndOpenSession = useCallback(
    async (cwd: string, sessionId: string, mode?: AgentMode) => {
      ...
      try {
        const poolData = await connectSession(HTTP_BASE, { cwd, mode: resolvedMode, sessionId });
        setSessionCreating(false);
  ...
  const handleSignOut = useCallback(async () => {
    await doLogout(HTTP_BASE).catch(() => null);
  ...
  const handleSaveApiKey = useCallback(async () => {
    ...
    try {
      await saveApiKey(HTTP_BASE, 'BZ_API_KEY', apiKeyValue.trim());
      setApiKeyValue('');
      retrySessionCreate();
    } finally {
  ...
  const sendRaw = useCallback((msg: object) => {
    ...
    if (!sid) return;
    sendMessageToAgent(HTTP_BASE, sid, msg).catch(() => null);
  }, []);
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        // Step 1: POST /api/pool/connect (fused: create-if-new + connect + messages)
        // connectSession throws on 401 (re-thrown below) or other errors.
        const connData = await connectSession(HTTP_BASE, connectParams as { cwd: string; mode: string; sessionId?: string });
        if (cancelled) return;
        ...
        const streamResp = await openEventsStream(HTTP_BASE, sid, abortController.signal);
        if (!streamResp.body) throw new Error('SSE stream failed');
        ...
        if (pendingAutoSendRef.current !== null && !cancelled) {
          ...
          const doSend = () => {
            ...
            sendMessageToAgent(HTTP_BASE, sid, { type: 'user', content }).catch(() => null);
          };
        ...
        if (cancelled) return;
        // 401 from connectSession → redirect to login
  ...
  const saveTitle = useCallback(
    (title: string) => {
      ...
      setIsEditingTitle(false);
      renameSession(HTTP_BASE, activeSessionId, trimmed).catch(() => null);
    },
    [activeSessionId],
  );
