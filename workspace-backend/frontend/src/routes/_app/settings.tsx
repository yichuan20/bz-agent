  ...
  async function handleSave() {
    ...
    try {
      for (const [key, value] of Object.entries(values)) {
        ...
        await apiSetCredential(HTTP_BASE, key, value.trim());
      }
      setSavedOk(true);
      setTimeout(() => {
        setSavedOk(false);
...
function CustomCredentials({ credKeys, onRefresh }: { credKeys: string[]; onRefresh: () => void }) {
  ...
  async function handleAdd() {
    ...
    setSaving(true);
    await apiSetCredential(HTTP_BASE, newKey.trim(), newVal.trim()).catch(() => null);
    setNewKey('');
    setNewVal('');
    setSaving(false);
    onRefresh();
  ...
  async function handleDelete(key: string) {
    await apiDeleteCredential(HTTP_BASE, key).catch(() => null);
    onRefresh();
  }
...
function IntegrationsSection() {
  ...
  const loadKeys = useCallback(() => {
    apiListCredentials(HTTP_BASE)
      .then((d) => setCredKeys(d.keys ?? []))
      .catch(() => null);
  }, []);
...
function ApiKeySection() {
  ...
  const loadStatus = useCallback(() => {
    apiGetApiKeyStatus(HTTP_BASE)
      .then((d) => setStatus({ present: d.present, last4: null }))
      .catch(() => null);
  }, []);
  ...
  async function handleSave() {
    ...
    setSavedOk(false);
    try {
      const r = await apiSetApiKey(HTTP_BASE, value.trim());
  ...
  async function handleDelete() {
    await apiDeleteApiKey(HTTP_BASE).catch(() => null);
    loadStatus();
  }
