...
const onMoveSelectedBox = (dx, dy, config, setConfig) => {
  ...
  setConfig({
    ...config,
    boxes: config.boxes.map(b => (b.isSelected ? { ...b, x: b.x + dx / SF, y: b.y + dy / SF } : b)),
  });
};
...
const onResizeSelectedBox = (offsetX, offsetY, handleType, config, setConfig) => {
  ...
  setConfig({
    ...config,
    boxes: config.boxes.map(b => (b.isSelected ? { ...b, x: nx, y: ny, w: nw, h: nh } : b)),
  });
};
...
const drawResizeHandles = (ctx, config) => {
  ...
  const corners = [
    ...
    { x: config.x + config.w, y: config.y },
    { x: config.x, y: config.y + config.h },
    { x: config.x + config.w, y: config.y + config.h },
  ];
  const edges = [
  ...
  corners.forEach(h => {
    ctx.beginPath();
    ctx.arc(h.x, h.y, 5 * SF, 0, Math.PI * 2);
  ...
  const rhx = config.x + config.w / 2,
    rhy = config.y - 28 * SF;
  ctx.beginPath();
  ctx.moveTo(rhx, config.y);
  ctx.lineTo(rhx, rhy + 8 * SF);
  ctx.strokeStyle = SEL_COLOR;
  ctx.lineWidth = 1.5 * SF;
  ctx.stroke();
  ctx.beginPath();
  ctx.arc(rhx, rhy, 8 * SF, 0, Math.PI * 2);
  ctx.fillStyle = '#ffffff';
  ctx.fill();
  ctx.lineWidth = 2 * SF;
  ctx.strokeStyle = SEL_COLOR;
  ctx.stroke();
  ctx.beginPath();
  ctx.arc(rhx, rhy, 4 * SF, -Math.PI * 0.9, Math.PI * 0.25);
  ctx.strokeStyle = SEL_COLOR;
  ctx.lineWidth = 1.5 * SF;
  ctx.stroke();
  const aEnd = Math.PI * 0.25,
    ar = 4 * SF;
  const ax = rhx + Math.cos(aEnd) * ar,
    ay = rhy + Math.sin(aEnd) * ar;
...
const drawTextBox = (ctx, config, isHovered = false, ptScale = PPTX_PT_SCALE, editState = null) => {
  ...
  const buildPath = () => {
    ...
    if (st === 'ellipse') {
      ...
      ctx.moveTo(x + aw, y);
      ctx.lineTo(x + 2 * aw, y);
      ctx.lineTo(x + 2 * aw, y + ah);
      ctx.lineTo(x + w, y + ah);
      ctx.lineTo(x + w, y + 2 * ah);
      ctx.lineTo(x + 2 * aw, y + 2 * ah);
      ctx.lineTo(x + 2 * aw, y + h);
      ctx.lineTo(x + aw, y + h);
      ctx.lineTo(x + aw, y + 2 * ah);
      ctx.lineTo(x, y + 2 * ah);
      ctx.lineTo(x, y + ah);
      ...
      ctx.moveTo(x, y + t);
      ctx.lineTo(x + t, y);
      ctx.lineTo(x + w / 2, y + h / 2 - t);
      ctx.lineTo(x + w - t, y);
      ctx.lineTo(x + w, y + t);
      ctx.lineTo(x + w / 2 + t, y + h / 2);
      ...
      const sw2 = h * 0.32,
        nx = x + w * 0.58;
      ctx.moveTo(x, y + h / 2 - sw2);
      ctx.lineTo(nx, y + h / 2 - sw2);
      ctx.lineTo(nx, y);
      ctx.lineTo(x + w, y + h / 2);
      ctx.lineTo(nx, y + h);
      ctx.lineTo(nx, y + h / 2 + sw2);
      ctx.lineTo(x, y + h / 2 + sw2);
      ctx.closePath();
    } else if (st === 'leftArrow') {
      const sw2 = h * 0.32,
        nx = x + w * 0.42;
      ctx.moveTo(x + w, y + h / 2 - sw2);
      ctx.lineTo(nx, y + h / 2 - sw2);
      ctx.lineTo(nx, y);
      ctx.lineTo(x, y + h / 2);
      ctx.lineTo(nx, y + h);
      ctx.lineTo(nx, y + h / 2 + sw2);
      ctx.lineTo(x + w, y + h / 2 + sw2);
      ctx.closePath();
    } else if (st === 'upArrow') {
      const sw2 = w * 0.32,
        ny = y + h * 0.42;
      ctx.moveTo(x + w / 2 - sw2, y + h);
      ctx.lineTo(x + w / 2 - sw2, ny);
      ctx.lineTo(x, ny);
      ctx.lineTo(x + w / 2, y);
      ctx.lineTo(x + w, ny);
      ctx.lineTo(x + w / 2 + sw2, ny);
      ctx.lineTo(x + w / 2 + sw2, y + h);
      ctx.closePath();
    } else if (st === 'downArrow') {
      const sw2 = w * 0.32,
        ny = y + h * 0.58;
      ctx.moveTo(x + w / 2 - sw2, y);
      ctx.lineTo(x + w / 2 - sw2, ny);
      ctx.lineTo(x, ny);
      ctx.lineTo(x + w / 2, y + h);
      ctx.lineTo(x + w, ny);
      ctx.lineTo(x + w / 2 + sw2, ny);
      ctx.lineTo(x + w / 2 + sw2, y);
      ctx.closePath();
    } else if (st === 'chevron') {
      const tip = w * 0.25;
      ctx.moveTo(x, y);
      ...
      ctx.moveTo(x, y);
      ctx.lineTo(x + w - cr2, y);
      ctx.lineTo(x + w, y + cr2);
      ctx.lineTo(x + w, y + h);
      ctx.lineTo(x, y + h);
      ctx.closePath();
      ...
      for (let i = 0; i < 10; i++) {
        const a = (i * Math.PI) / 5 - Math.PI / 2,
          r2 = i % 2 === 0 ? ro : ri;
        i === 0
          ? ctx.moveTo(cx2 + r2 * Math.cos(a), cy2 + r2 * Math.sin(a))
  ...
  if (config.imageData) {
    ...
    try {
      ctx.drawImage(_imgCache.get(config.id), x, y, w, h);
    } catch {}
  ...
  if (config.canvasImage || isBase64Image(config.text)) {
    ...
    if (!img && isBase64Image(config.text)) {
      if (_imgCache.has(config.id)) {
        ...
        const ni = new Image();
        ni.onload = () => {
          _imgCache.set(config.id, ni);
          config.canvasImage = ni;
        };
        ni.src = config.text;
  ...
  if (sc) {
    ctx.fillStyle = sc.bgColor || '#1473df';
    ctx.strokeStyle = sc.borderColor || '#0d5bb5';
  ...
  const bgColor =
    fill?.type === 'solid' && fill.color ? fill.color : config.boxStyle?.bgColor || 'transparent';
  if (bgGrad && bgGrad.stops && bgGrad.stops.length >= 2) {
    ...
    const lg = ctx.createLinearGradient(
      cx2 - Math.cos(rad2) * len2,
      cy2 - Math.sin(rad2) * len2,
      cx2 + Math.cos(rad2) * len2,
      cy2 + Math.sin(rad2) * len2,
    );
  ...
  if ((config.shapeType || 'rect') === 'cube') {
    ...
    ctx.beginPath();
    ctx.moveTo(x, y + d2);
    ctx.lineTo(x + w - d2, y + d2);
    ctx.lineTo(x + w, y);
    ctx.moveTo(x + w - d2, y + d2);
    ctx.lineTo(x + w - d2, y + h);
    ctx.strokeStyle = lc;
    ctx.lineWidth = Math.max(1, borderWidth || 1 * SF);
    ctx.stroke();
  }
  ...
  if (hasParagraphs) {
        ...
        for (let pi = 0; pi < parts.length; pi++) {
          ...
          for (let wi = 0; wi < words.length; wi++) {
            ...
            if (lineW + candW > maxW + wrapTolerance && (segs.length > 0 || pending)) {
              if (pending) {
                const pw = measure(pending);
                segs.push({ text: pending, color, fs, ff, fw, fi, ls, w: pw });
                lineW += pw;
              }
              flush();
              lineH = Math.max(lineH, lh);
    ...
    for (let i = 0; i < text.length; i++) {
      const ch = text[i];
      if (ch === '\n') {
        lines.push({ chars: curChars, nlCi: i });
        curChars = [];
        tempCx = x + pad;
        continue;
      }
    ...
    for (let li = 0; li < lines.length; li++) {
      ...
      if (!caretDone && firstCi === caretPos && caretVis) {
        drawCaret(cx, cy);
        caretDone = true;
      }
      ...
      for (const { ci, ch, cs, chW, font: chFont } of lc) {
        ...
        if (hasSel && ci >= selStart && ci < selEnd) {
          ...
          ctx.fillRect(cx, cy, chW, lineHeight);
          ctx.restore();
          ctx.fillStyle = fc;
        }
        ...
        if (cs.textDecoration === 'underline') {
          ctx.save();
          ctx.strokeStyle = fc;
          ctx.lineWidth = Math.max(1, scaledFS * 0.05);
          ctx.beginPath();
          ctx.moveTo(cx, cy + scaledFS * 1.05);
          ctx.lineTo(cx + chW, cy + scaledFS * 1.05);
          ctx.stroke();
          ctx.restore();
        }
        cx += chW;
        if (!caretDone && ci + 1 === caretPos && caretVis) {
          drawCaret(cx, cy);
          caretDone = true;
        }
      }
      ...
      if (nlCi >= 0) {
        if (hasSel && nlCi >= selStart && nlCi < selEnd) {
          ctx.save();
          ctx.fillStyle = '#0b57d033';
          ctx.fillRect(cx, cy, scaledFS * 0.35, lineHeight);
          ctx.restore();
        }
        if (!caretDone && nlCi === caretPos && caretVis) {
          drawCaret(cx, cy);
          caretDone = true;
        }
      }
...
export const drawConfig = (
  ...
  sc.boxes.forEach(box => {
    ...
    if (isPresentationMode) {
      drawTextBox(ctx, { ...box, isSelected: false }, false, _ptScale, null);
      return;
    }
    if (box.isSelected && isDragging) {
      ...
      drawTextBox(
        ctx,
        { ...box, x: box.x + dragOffset.x * SF, y: box.y + dragOffset.y * SF },
        false,
        _ptScale,
        boxEdit,
      );
...
const getLocationForBox = (mouseX, mouseY, box) => {
  ...
  if (near(x, y)) return 'topLeftCorner';
  if (near(x + w, y)) return 'topRightCorner';
  if (near(x, y + h)) return 'bottomLeftCorner';
  if (near(x + w, y + h)) return 'bottomRightCorner';
  if (near(x + w / 2, y)) return 'topEdge';
  if (near(x + w / 2, y + h)) return 'bottomEdge';
  if (near(x, y + h / 2)) return 'leftEdge';
  if (near(x + w, y + h / 2)) return 'rightEdge';
  const bp = 10 * SF;
  if (
    inRange(mouseX, x - GRAB_DISTANCE, x + bp) ||
    inRange(mouseX, x + w - bp, x + w + GRAB_DISTANCE) ||
    inRange(mouseY, y - GRAB_DISTANCE, y + bp) ||
    inRange(mouseY, y + h - bp, y + h + GRAB_DISTANCE)
  )
    return 'border';
  return 'inside';
};
...
const getBoxUnderMouse = (e, config, ctx) => {
  ...
  const hits = sc.boxes.filter(b => {
    const dx = b.w < 0 ? -1 : 1,
      dy = b.h < 0 ? -1 : 1;
    return (
      inRange(mx, b.x - GRAB_DISTANCE * dx, b.x + b.w + GRAB_DISTANCE * dx) &&
...
const CtxShortcut = ({ children }) => (
  <span style={{ fontSize: 11, color: 'var(--text-secondary)', marginLeft: 20 }}>{children}</span>
);
const CtxDivider = () => (
  ...
  return (
    ...
    >
      <CtxItem style={{ justifyContent: 'space-between' }}>
        {label}
        <span style={{ fontSize: 10, color: 'var(--text-secondary)' }}>{flipLeft ? '◀' : '▶'}</span>
      </CtxItem>
      {open && (
        <div
          style={{
...
const Slide = ({
  ...
  const handleCut = useCallback(() => {
    ...
    if (sel) {
      setClipboard(cloneDeep(sel));
      onDeleteSelectedBox(configRef.current, setConfig);
    }
  ...
  const handleBringToFront = useCallback(() => {
    const nc = cloneDeep(configRef.current);
    const i = nc.boxes.findIndex(b => b.isSelected);
    if (i !== -1) {
      const [box] = nc.boxes.splice(i, 1);
      nc.boxes.push(box);
      setConfig(nc);
    }
    setContextMenu({ visible: false, x: 0, y: 0 });
  ...
  const handleBringForward = useCallback(() => {
    const nc = cloneDeep(configRef.current);
    const i = nc.boxes.findIndex(b => b.isSelected);
    if (i !== -1 && i < nc.boxes.length - 1) {
      const [box] = nc.boxes.splice(i, 1);
      nc.boxes.splice(i + 1, 0, box);
      setConfig(nc);
    }
    setContextMenu({ visible: false, x: 0, y: 0 });
  }, [setConfig]);
  ...
  useEffect(() => {
      ...
      if (eid) {
        ...
        if (e.key === 'ArrowLeft' && !e.metaKey && !e.ctrlKey) {
          ...
          if (e.shiftKey) {
            ...
            setCaretPos(np);
            caretPosRef.current = np; // selAnchor stays
          } else {
            const np = hasSel ? sStart : Math.max(0, cp - 1);
            setCaretPos(np);
            setSelAnchor(np);
            caretPosRef.current = np;
            selAnchorRef.current = np;
          }
          return;
        }
        ...
        if (e.key === 'Backspace') {
          ...
          if (hasSel) {
            ...
            setCaretPos(sStart);
            setSelAnchor(sStart);
            caretPosRef.current = sStart;
            selAnchorRef.current = sStart;
          } else {
            if (cp === 0) return;
    ...
    const onPaste = e => {
      ...
      if (!eid && html?.includes('<img')) {
        insertBoxesFromHtml(html, cfg, setConfig);
        return;
      }
      if (eid) {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: JSON.stringify used for deep compare
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const rafId = requestAnimationFrame(() => {
      try {
        onEditEnd(canvas.toDataURL('image/jpeg', 0.5));
      } catch {}
    });
  ...
  return (
    ...
    >
      <canvas
        ...
        onMouseMove={e => {
          if (isPresentationMode) {
            canvasRef.current.style.cursor = defaultCursor;
            return;
          }
          ...
          canvasRef.current.style.cursor =
            isImg && loc === 'inside' ? 'move' : locationToCursor[loc] || defaultCursor;
          // Drag-to-select while in text edit mode
          if (e.buttons === 1 && editBoxId) {
            const [, , ci] = getBoxUnderMouse(e, config, ctx);
            if (ci !== caretPosRef.current) {
              setCaretPos(ci);
              caretPosRef.current = ci;
            }
            return;
          }
          ...
          if (
            (resizes.includes(loc) || currentAction === 'resizing') &&
          ...
          if (loc === 'inside' || loc === 'border' || currentAction === 'moving') {
            ...
            dragOffsetRef.current = {
              x: dragOffsetRef.current.x + movementX * cs,
              y: dragOffsetRef.current.y + movementY * cs,
            };
            const c = canvasRef.current?.getContext('2d');
            if (c)
              drawConfig(c, config, dragOffsetRef.current, isPresentationMode, hoveredBoxId, null);
        ...
        onMouseDown={e => {
          ...
          if (!box) {
            ...
            if (activeTool === 'shape') {
              ...
              nc.boxes.push({
                id: uuidv4(),
                x: offsetX * _cs - size / 2,
                y: offsetY * _cs - size / 2,
                w: size,
                h: size,
                shapeType: activeShape,
                fill: { type: 'solid', color: '#1473df' },
                text: '',
                styles: [],
                boxStyle: { borderColor: '#0d5bb5', borderWidth: 2, bgColor: 'transparent' },
                isSelected: true,
              });
              onSave(nc);
              return;
            ...
            if (activeTool === 'text') {
              onAddNewBox(
                e.nativeEvent.offsetX * _cs,
                e.nativeEvent.offsetY * _cs,
                'Add text here',
                config,
                setConfig,
              );
          ...
          if (curSel?.id !== box.id) {
            if (editBoxId) exitEdit();
            onSetSelectedBox(box.id, config, setConfig);
            return;
          }
          ...
          if (editBoxId === box.id) {
            ...
            if (e.detail >= 3) {
              ...
              const t = box.text || '';
              setCaretPos(t.length);
              setSelAnchor(0);
              caretPosRef.current = t.length;
              selAnchorRef.current = 0;
            } else {
        ...
        onMouseUp={() => {
          if (isDraggingRef.current && (dragOffsetRef.current.x || dragOffsetRef.current.y)) {
            onMoveSelectedBox(
              dragOffsetRef.current.x * SF,
              dragOffsetRef.current.y * SF,
              config,
              setConfig,
            );
          }
        ...
        onDoubleClick={e => {
          ...
          if (
            box.imageData ||
            isBase64Image(box.text) ||
            box.canvasImage ||
            isShapeConfig(box.text)
          )
            return;
          ...
          if (editBoxId === box.id) {
            ...
            setCaretPos(we);
            setSelAnchor(ws);
            caretPosRef.current = we;
            selAnchorRef.current = ws;
            return;
          }
        ...
        createPortal(
          (() => {
            ...
            return (
              ...
              >
                <CtxItem disabled={!selectedBox} onClick={handleCut}>
                  Cut<CtxShortcut>⌘X</CtxShortcut>
                </CtxItem>
                <CtxItem disabled={!selectedBox} onClick={handleCopy}>
                  Copy<CtxShortcut>⌘C</CtxShortcut>
                </CtxItem>
                <CtxItem disabled={!clipboard} onClick={handlePaste}>
                  Paste<CtxShortcut>⌘V</CtxShortcut>
                </CtxItem>
                <CtxDivider />
                <CtxItem disabled={!selectedBox} onClick={handleDelete}>
                  Delete<CtxShortcut>⌫</CtxShortcut>
                </CtxItem>
                <CtxDivider />
