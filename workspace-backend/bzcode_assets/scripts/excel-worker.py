...
def _call(fname: str, args_str: str, grid: Grid, grids: "dict[str, Grid] | None" = None):
    ...
    if fname == "IFERROR":
        try:
            return _eval(raw_args[0], grid, grids) if raw_args else ""
        except Exception:
            return _eval(raw_args[1], grid, grids) if len(raw_args) > 1 else ""
...
def compute_sheet(sheet_def: dict) -> dict:
    ...
    sorted_formulas = sorted(
        formulas.items(), key=lambda kv: _parse_ref(kv[0].strip().upper()) if _is_valid_ref(kv[0]) else (0, 0)
    )
    ...
    def _get_fmt(s: dict | None):
        ...
        if key not in _fmt_cache:
            ...
            if s.get("bg"):
                props["bg_color"] = s["bg"]
            if s.get("fg"):
                props["font_color"] = s["fg"]
            if s.get("format"):
                props["num_format"] = s["format"]
            if s.get("wrap"):
                props["text_wrap"] = True
            if s.get("fontSize"):
                props["font_size"] = s["fontSize"]
            if s.get("align"):
